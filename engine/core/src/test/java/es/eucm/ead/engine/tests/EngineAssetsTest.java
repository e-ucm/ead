...
public class EngineAssetsTest {
	...
	public void setUp() throws IOException {
		new MockApplication();
		engineAssets = new EngineAssets(new MockFiles());
		...
		engineAssets.setLoadingPath(gameFolder.file().getAbsolutePath(), false);
	}

	@Test
	public void testAbsolutePath() {
		FileHandle gameResolved = engineAssets.resolve(gameFile.file()
				.getAbsolutePath());
	...
	public void testGameProjectPath() {
		FileHandle gameResolved = engineAssets.resolve("game.json");
		assertTrue(gameResolved.exists());
		assertEquals(CONTENT, gameResolved.readString());
	}
	...
	public void testGameProjectPathFallback() {
		...
		FileHandle fileResolved = engineAssets.resolve("bindings.json");
	...
	public void testDefaultFont() {
		assertNotNull(engineAssets.getDefaultFont());
	}

	@Test
	public void testGamePathNull() {
		// Assets must be able to access files with game path set to null
		engineAssets.setLoadingPath(null, false);
		assertTrue(engineAssets.resolve("bindings.json").exists());
	}

	@Test
	public void testGamePathCorrected() {
		String path = "path";
		engineAssets.setLoadingPath(path, false);
		assertEquals(engineAssets.getLoadingPath(), path + "/");
		assertFalse(engineAssets.isGamePathInternal());
	}

	@Test
	public void testGetEngineObject() {
		engineAssets.bind("schemaobject", SchemaObject.class,
				EngineObjectMock.class);
		SchemaObject schemaObject = new SchemaObject();
		assertEquals(engineAssets.getEngineObject(schemaObject).getClass(),
				EngineObjectMock.class);
		assertNull(engineAssets.getEngineObject(Object.class));
	}

	@Test
	public void testLoadBindings() {
		try {
			engineAssets.loadBindings(engineAssets.resolve("bindings.json"));
		} catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
		}
