...
public class SceneElementActor extends AbstractActor<SceneElement> {
    ...
    /**
     * This method is either used by {@link #readRenderer(es.eucm.ead.schema.actors.SceneElement)} to setup the default renderer and also by effect Change renderer {@link es.eucm.ead.engine.actions.ChangeRendererAction}
     * The renderer will only be changed if newRenderer is different from the current one (!newRenderer.equals(renderer))
     * @param newRenderer   The new renderer to be setup. It can either by a new Renderer or element.getRenderer()
    ...
    /**
     * This method just restores the original renderer this sceneleement used to have. To be invoked by {@link es.eucm.ead.engine.actions.ChangeRendererAction}
     * @return True if the renderer actually changed, false otherwise
     */
