...
public class TextEngineObject extends RendererEngineObject<Text> {
	...
	public void initialize(Text schemaObject) {
		text = gameLoop.getGameAssets().getI18N().m(schemaObject.getText());
		style = null;
		...
		// use this one
			...
			try {
				style = gameLoop.getGameAssets().get(styleRefPath,
						TextStyle.class);
			}
		...
		if (style == null) {
			try {
				style = gameLoop.getGameAssets().get(DEFAULT_TEXT_STYLE_PATH,
						TextStyle.class);
			}
		...
		if (style == null) {
			...
			scale = 1.0F;
			color = Color.WHITE;
			bitmapFont = gameLoop.getGameAssets().getDefaultFont();
		} else {
			...
			if (fontFile == null) {
				...
				bitmapFont = gameLoop.getGameAssets().get(style.getFont(),
						BitmapFont.class);
			}
		}
