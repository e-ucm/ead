...
public class GeoTester {
	...
	public static Pixmap randomPixmap(int width, int height, Pixmap.Format fmt) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				p.drawPixel(x, y, Color.rgba8888(r(0, 256), r(0, 256),
						r(0, 256), 128));
			}
		}
		return p;
	 ...
	 * x-axis
	 *
	 * @param n the number of polygons to create
	 * @param distance between polygon centers
	 * @param size of polygons (will vary by 10% up/down)
	 * @param offset initial polygon located here
	 * @param target array where they will be written
	 */
	...
	public static class GeoViewer implements ApplicationListener {
		...
		public void create() {
			...
			// create a string of generally-overlapping polygons, will draw in blue
				...
				updatedRegion = executor.submit(new AsyncTask<PolygonRegion>() {
					...
					public PolygonRegion call() throws Exception {
						...
						while (!pendingRemoves.isEmpty()) {
							GeometryUtils.substract(geo, pendingRemoves.poll());
						}
						Geometry collapsed = GeometryUtils.collapse(geo);
						if (r(0, 10) < 4) {
							GeometryUtils.simplify(geo, 3);
	...
	public static void main(String args[]) {
		...
		config.width = 1000;
		config.height = 800;
		new LwjglApplication(new GeoViewer());
	}
}
