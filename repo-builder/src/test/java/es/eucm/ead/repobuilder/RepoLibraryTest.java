...
/**
 * Simple test class for the RepoBuilder. Automatically creates the library and gets it exported and extracted to a temp folder ({@code outputFolder}). It checks that subfolders elements/, libraries/ and preview/ are created.
 *
...
public abstract class RepoLibraryTest extends RepoLibraryBuilder{
    ...
    /**
     * Path to the test image included in test.zip. This file will be accessible if the default test file is used (when the constructor with no arguments is used).
     */
    ...
    /**
     * Array containing any temporary (FileHandle.tempDirectory) directories and files created during the test that have to be cleaned up upon completion.
     */
    protected Array<FileHandle> tempFolders;

    /**
     * Initializes the test with the contents of file "test.zip", which just contains a 2x2 white png image called "image.png".
     */
    ...
    /**
     * Initializes the test with the contents of the zip file especified by {@code root}.
     * @param root  Name of the zip file containing the images and other files required for the test. Files root.zip and root.png must be available under test/resources.
     */
    ...
    public void setup(){
        ...
        setCommonProperty(RepoLibraryBuilder.VERSION,
                "testversion");
        ...
        assertTrue("Elements sub-folder does not exist or it's not a directory",
                elementsSubFolder.exists() && elementsSubFolder.isDirectory());

        FileHandle librariesFile = outputFolder.child("libraries");
        assertTrue("Libraries sub-folder does not exist or it's not a directory",
    ...
    public void checkNumberOfElements(int expectedNumberOfElements){
        FileHandle elementsSubFolder = outputFolder.child("elements");
        assertTrue("Elements sub folder does not exist or it's not a directory", elementsSubFolder.exists() && elementsSubFolder.isDirectory());
        assertEquals("The number of expected elements does not match", expectedNumberOfElements, elementsSubFolder.list().length);
    }

    /**
     * Checks that a zip file with the given elementName has been produced and has a valid structure:
     * thumbnails/
     * contents.zip
     * descriptor.json
     *
     ...
     * @param elementName
     * @return  The temp folder where the element file is extracted to, so it can be used to make further checks if necessary
     */
    public FileHandle checkElement(String elementName){
        ...
        assertTrue("Element zip file does not exist or it's not a zip file", elementZip.exists() && !elementZip.isDirectory());

        // Unzip the file
        FileHandle extractedElementFolder = FileHandle.tempDirectory("test-element");
        ZipUtils.unzip(elementZip, extractedElementFolder);
        // Check there are a three things: thumbnails/, contents.zip, descriptor.json
        ...
        assertTrue("Thumbnails folder does not exist or it's not valid", thumbnails.exists() && thumbnails.isDirectory());
        assertTrue("Contents zip does not exist or it's not valid", contents.exists() && !contents.isDirectory() && contents.length()>0);
        assertTrue("Descriptor does not exist or it's not valid", descriptor.exists() && !descriptor.isDirectory() && descriptor.length()>0);

        // Thumbnails check
        for (FileHandle thumbnail:thumbnails.list()){
            assertTrue("Not valid thumbnail: " + thumbnail.name(),
                    thumbnail.exists() && !thumbnail.isDirectory() && thumbnail.length() > 0 && thumbnail.name().toLowerCase().endsWith(".png") && thumbnail.nameWithoutExtension().matches("\\d+x\\d+"));
        }
    ...
    /**
     * Checks that the contents.zip file of element in {@code elementFolder} has the entries provided as inputs
     * @param elementFolder The folder where the element was extracted to
     * @param entries   The list of entries to be checked to be present in the file
    ...
    public FileHandle checkContents(FileHandle elementFolder, String...entries){
        ...
        assertEquals("Number of expected entries don't match", entries.length, ZipUtils.listEntries(elementFolder.child("contents.zip")).size);
