...
import es.eucm.ead.editor.control.actions.ExportGame;
 ...
 * command shell and batch exportation more than for testing, although it uses
 * the mock testing infrastructure.
 * 
 ...
 * it just loads the game project using an
 * {@link es.eucm.ead.editor.control.actions.OpenGame} action and then it
 * creates an {@link es.eucm.ead.editor.control.actions.ExportGame} action.
 * 
...
public class Exporter {
	...
	/**
	 * Attributes used for synchronization - the exportation process waits for
	 * the {@link es.eucm.ead.editor.control.actions.OpenGame} action to
	 * complete before going forward with the exportation. Then it waits for
	 * {@link es.eucm.ead.editor.control.actions.ExportGame} before returning.
	 */
	private static Object lock;
	private static boolean loaded;
	private static boolean exported;
	private static boolean export_completed;
	...
	public static boolean exportAsJar(String projectPath, String engineJarPath,
		...
		Model mockModel = mockController.getModel();
		lock = new Object();
		mockModel.addLoadListener(new Model.ModelListener<LoadEvent>() {
			@Override
		...
		// Load the game
		mockController.action(OpenGame.class, projectPath);
		...
		// Export
		mockController.action(ExportGame.class, destinyPath, engineJarPath,
				new EditorIO.ExportCallback() {
					@Override
		...
		// Wait till exportation completes
		synchronized (lock) {
			if (!export_completed) {
				lock.wait();
			}
		...
		if (help || engineLibPath == null || projectPath == null
			...
			if (projects.size() != targets.size()) {
				...
				for (int i = 0; i < projects.size(); i++) {
					try {
						Exporter.exportAsJar(projects.get(i), engineLibPath,
								targets.get(i));
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
	...
	private static void printUsage() {
		...
		System.out
				.println("\t\tjava es.eucm.ead.editor.control.Exporter -engine-lib [PATH TO THE ENGINE LIB] -project [PATH TO THE GAME PROJECT] -target [PATH TO THE JAR OUTPUT]");
		System.out.println("\tOr:");
		System.out
				.println("\t\tjava es.eucm.ead.editor.control.Exporter -engine-lib [PATH TO THE ENGINE LIB] -project-list [COMMA-SEPARATED PATHS TO THE GAME PROJECTS] -target-list [COMMA-SEPARATED PATHS TO THE JAR OUTPUTS]");
		System.out.println();
		System.out.println("\tExamples:");
		System.out
				.println("\t\tjava es.eucm.ead.editor.control.Exporter -engine-lib \"C:/Users/Javier Torrente/.m2/repository/es/e-ucm/ead/engine-desktop/1.0-SNAPSHOT/engine-desktop-1.0-SNAPSHOT-jar-with-dependencies.jar\" -project \"C:\\Users\\Javier Torrente\\GIT_REPOS\\ead\\engine\\desktop\\src\\test\\"
						+ "resources\\techdemo\\\" -target \"C:\\Users\\Javier Torrente\\Downloads\\Exports\\techdemo.jar\"");
		System.out
				.println("\t\tjava es.eucm.ead.editor.control.Exporter -engine-lib \"C:/Users/Javier Torrente/.m2/repository/es/e-ucm/ead/engine-desktop/1.0-SNAPSHOT/engine-desktop-1.0-SNAPSHOT-jar-with-dependencies.jar\" -project-list \"C:\\Users\\Javier Torrente\\GIT_REPOS\\ead\\engine\\desktop\\src\\test\\resources\\techdemo,C:\\Users\\Javier Torrente\\GIT_REPOS\\parity2.0\\src\\main\\resources\\parity2.0\" -target-list \"C:\\Users\\Javier Torrente\\Downloads\\Exports\\techdemo.jar,C:\\Users\\Javier Torrente\\Downloads\\Exports\\parity.jar\"");
	}
}
