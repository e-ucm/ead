...
public class EditorAssetsTest {
	...
	@Before
	public void setUp() {
		editorAssets = new EditorAssets(files, new ApplicationAssets(files));
		projectFolder = platform.createTempFile(true);
		editorAssets.setLoadingPath(projectFolder.getAbsolutePath(), true);
	}
	...
	public void testCopyAndLoad() {
		...
		assertEquals(editorAssets.copyAndLoad(externalFile.getAbsolutePath(),
				Texture.class),
				EditorAssets.IMAGES_FOLDER + externalFile.getName());

		String newPath = EditorAssets.IMAGES_FOLDER + externalFile.getName();
		assertTrue(editorAssets.resolve(newPath).exists());
		...
		for (int i = 1; i < 10; i++) {
			newPath = EditorAssets.IMAGES_FOLDER + externalFile.getName() + i;
			assertEquals(editorAssets.copyAndLoad(
	 ...
	 * {@link #editorAssets} by reflection.
	 * 
	 * This should only be done for testing.
	 * 
	 * To access the assetManager through an
	 * {@link es.eucm.ead.editor.assets.EditorAssets}, it first retrieves its
	 * superclass ({@link es.eucm.ead.engine.EngineAssets}), and then also its
	...
	private AssetManager getAssetManagerForTesting() {
		...
		try {
			Field assetManager = EditorAssets.class.getSuperclass()
					.getSuperclass().getDeclaredField("assetManager");
			assetManager.setAccessible(true);
			am = (AssetManager) assetManager.get(editorAssets);
			...
			assertFalse(
					"An exception was thrown while retrieving the Assets.assetManager for testing. Exception: "
							+ e.toString(), true);
			Gdx.app.debug("Test failed: ", "EditorAssetsTest", e);
