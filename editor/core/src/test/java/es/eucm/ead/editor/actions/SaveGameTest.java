...
public class SaveGameTest extends EditorTest {
	...
	public void testSaveAll() {
		...
		mockController.getEditorAssets().setLoadingPath(gameFolderPath);
		...
		testFileExists(gameFolderPath, EditorAssets.GAME_FILE);
		for (int i = 0; i < 5; i++) {
			testFileExists(gameFolderPath, EditorAssets.SCENES_PATH + "scene"
					+ i + ".json");
		}
		...
		File imagesDir = new File(gameFolderPath, EditorAssets.IMAGES_FOLDER);
		imagesDir.mkdirs();
		File imageFile = null;
		try {
		...
		testFileExists(gameFolderPath, EditorAssets.GAME_FILE);
		testFileExists(gameFolderPath, EditorAssets.SCENES_PATH + "scene2.json");
		testFileExists(gameFolderPath, EditorAssets.SCENES_PATH + "scene3.json");
		testFileDoesNotExist(gameFolderPath, EditorAssets.SCENES_PATH
				+ "scene0.json");
		testFileDoesNotExist(gameFolderPath, EditorAssets.SCENES_PATH
				+ "scene1.json");
		testFileDoesNotExist(gameFolderPath, EditorAssets.SCENES_PATH
		...
		mockController.action(OpenGame.class, new File(gameFolderPath,
				EditorAssets.GAME_FILE).getAbsolutePath());
