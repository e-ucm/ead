...
import es.eucm.ead.editor.view.widgets.ScrollPane.ScrollPaneListener.Edge;
import es.eucm.ead.editor.view.widgets.ScrollPane.ScrollPaneListener.ScrollPaneEvent;
...
public class ScrollPane extends WidgetGroup {
		...
		addCaptureListener(new InputListener() {
			...
			public void touchDragged(InputEvent event, float x, float y,
				...
				if (touchScrollH) {
					...
					scrollH = Math.min(hScrollBounds.x + hScrollBounds.width
							- hKnobBounds.width, scrollH);
					...
					scrollV = Math.min(vScrollBounds.y + vScrollBounds.height
							- vKnobBounds.height, scrollV);
	...
	void testHitXEdge() {
		...
		if (scrollX) {
			...
			// Right
			if (hitRight) {
				if (getScrollX() <= maxX + 1 && velocityX == 0
						&& velocityY == 0) {
					hitRight = false;
				}
			} else {
	...
	void testHitYEdge() {
		...
		if (scrollY) {
			...
			// Bottom
			if (hitBottom) {
				if (getScrollY() <= maxY + 1 && velocityX == 0
						&& velocityY == 0) {
					hitBottom = false;
				}
			} else {
	...
	void sendHitEdgeEvent(Edge edge) {
		ScrollPaneEvent event = Pools.obtain(ScrollPaneEvent.class);
		event.setEdge(edge);
		fire(event);
		Pools.free(edge);
	}
	...
	public void act(float delta) {
		...
		if (fadeAlpha > 0 && fadeScrollBars && !panning && !touchScrollH
				&& !touchScrollV) {
			fadeDelay -= delta;
			if (fadeDelay <= 0)
	...
	public void layout() {
		...
		if (style.hScroll != null)
			scrollbarHeight = Math.max(scrollbarHeight,
					style.hScroll.getMinHeight());
		float scrollbarWidth = 0;
		if (vScrollKnob != null)
			scrollbarWidth = vScrollKnob.getMinWidth();
		if (style.vScroll != null)
			scrollbarWidth = Math.max(scrollbarWidth,
					style.vScroll.getMinWidth());
		...
		if (!fade) {
			// Check again, now taking into account the area that's taken up by
			// any enabled scrollbars.
			if (scrollY) {
		...
		if (scrollX) {
			if (hScrollKnob != null) {
				float hScrollHeight = style.hScroll != null ? style.hScroll
						.getMinHeight() : hScrollKnob.getMinHeight();
				// The corner gap where the two scroll bars intersect might have
				// to flip from right to left.
				float boundsX = vScrollOnRight ? bgLeftWidth : bgLeftWidth
						+ scrollbarWidth;
				// Scrollbar on the top or bottom.
				float boundsY = hScrollOnBottom ? bgBottomHeight : height
						- bgTopHeight - hScrollHeight;
				hScrollBounds.set(boundsX, boundsY, areaWidth, hScrollHeight);
				if (variableSizeKnobs)
					hKnobBounds.width = Math
							.max(hScrollKnob.getMinWidth(),
		...
		if (scrollY) {
			if (vScrollKnob != null) {
				float vScrollWidth = style.vScroll != null ? style.vScroll
						.getMinWidth() : vScrollKnob.getMinWidth();
				// the small gap where the two scroll bars intersect might have
				// to flip from bottom to top
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		if (scrollX && scrollY) {
			if (style.corner != null) {
				style.corner.draw(batch, hScrollBounds.x + hScrollBounds.width,
						hScrollBounds.y, vScrollBounds.width, vScrollBounds.y);
			}
		}
		if (scrollX) {
			if (style.hScroll != null)
				style.hScroll.draw(batch, hScrollBounds.x, hScrollBounds.y,
						hScrollBounds.width, hScrollBounds.height);
			if (style.hScrollKnob != null)
				style.hScrollKnob.draw(batch, hKnobBounds.x, hKnobBounds.y,
						hKnobBounds.width, hKnobBounds.height);
		}
		if (scrollY) {
			if (style.vScroll != null)
				style.vScroll.draw(batch, vScrollBounds.x, vScrollBounds.y,
						vScrollBounds.width, vScrollBounds.height);
			if (style.vScrollKnob != null)
				style.vScrollKnob.draw(batch, vKnobBounds.x, vKnobBounds.y,
						vKnobBounds.width, vKnobBounds.height);
		}
	...
	/**
	 * Returns the x scroll position in pixels, where 0 is the left of the
	 * scroll pane.
	 */
	...
	/**
	 * Returns the y scroll position in pixels, where 0 is the top of the scroll
	 * pane.
	 */
	...
	public void setFlickScrollTapSquareSize(float halfTapSquareSize) {
		flickScrollListener.getGestureDetector().setTapSquareSize(
				halfTapSquareSize);
	}
	...
	public float getScrollBarHeight() {
		return style.hScrollKnob == null || !scrollX ? 0 : style.hScrollKnob
				.getMinHeight();
	}

	public float getScrollBarWidth() {
		return style.vScrollKnob == null || !scrollY ? 0 : style.vScrollKnob
				.getMinWidth();
	}
	...
	/**
	 * When false (the default), the widget is clipped so it is not drawn under
	 * the scrollbars. When true, the widget is clipped to the entire scroll
	 * pane bounds and the scrollbars are drawn on top of the widget. If
	 * {@link #setFadeScrollBars(boolean)} is true, the scroll bars are always
	 * drawn on top.
	 */
	...
	/**
	 * If true, the scroll knobs are sized based on {@link #getMaxX()} or
	 * {@link #getMaxY()}. If false, the scroll knobs are sized based on
	 * {@link Drawable#getMinWidth()} or {@link Drawable#getMinHeight()}.
	 * Default is true.
	 */
	...
	/**
	 * Listener for {@link ScrollPaneEvent} to when a scroll pane scrolls into
	 * an edge
	 * 
		...
		public boolean handle(Event event) {
			if (event instanceof ScrollPaneEvent) {
				if (event.getTarget() instanceof ScrollPane) {
					hitEdge((ScrollPane) event.getTarget(),
							((ScrollPaneEvent) event).getEdge());
				}
			}
			return false;
		}
