...
/**
 * Unlocks all actors in current scene.
 */
public class SetAllUnlocked extends ModelAction {
	...
	public Command perform(Object... args) {
		...
		if (sceneEntity != null) {
			...
			setAllUnlocked(scene.getGroup(), command);
		}
	...
	private void setAllUnlocked(Actor actor, CompositeCommand command) {
		ModelEntity entity = Q.getModelEntity(actor);
		if (entity != null) {
			ElementState lock = Q.getComponent(entity, ElementState.class);
			if (lock.isLock()) {
				command.addCommand(new FieldCommand(lock, FieldName.LOCK, false));
				command.addCommand(new FieldCommand(actor, FieldName.TOUCHABLE,
						Touchable.enabled));
			}
			if (actor instanceof Group) {
				for (Actor child : ((Group) actor).getChildren()) {
					setAllUnlocked(child, command);
				}
			}
		}
