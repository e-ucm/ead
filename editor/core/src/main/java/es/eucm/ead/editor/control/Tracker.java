...
public class Tracker {
    ...
    /**
     * The URL used to post bug reports. Should be provided by {@link es.eucm.ead.editor.control.Controller},
     * who has access to it through the {@link es.eucm.ead.editor.control.appdata.ReleaseInfo} file where
     * it is stored.
     */
    private String bugReportURL;

    /**
     * This object needs the controller to access {@link es.eucm.ead.editor.control.Actions#getLoggedActions(int)},
     * and {@link es.eucm.ead.editor.assets.ApplicationAssets#toJson(Object)} for retrieving and serializing
     * actions for bug reporting; and also {@link Controller#getRequestHelper()} to make requests to the bug
     * reporting backend.
     */
    private Controller controller;

    /**
     * Constructor. Receives the url of the backend used for bug reporting.
     * If {@link #bugReportURL} is null, the bug report feature will
     * be disabled.
     *
     * It also receives the controller to get access to some stuff (see
     * comment above).
     *
     */
    ...
    public void reportBug(Throwable e) {
        ...
        if (bugReportURL != null) {
            ...
            bugReport.setActionsLog(controller.getActions().getLoggedActions(ACTION_NUMBER_BUGREPORTING));
