 ...
 * functionality is provided via the {@link com.badlogic.gdx.utils.Clipboard}
 * interface. Currently there are two standard implementations, one for the
 * desktop and one for Android. The Android clipboard is a stub, as copy &
 * pasting on Android is not supported yet.
 * <p>
...
public class TextField extends Widget implements Disableable {
	...
	/**
	 * When false, text set by {@link #setText(String)} may contain characters
	 * not in the font, a space will be displayed instead. When true (the
	 * default), characters not in the font are stripped by setText. Characters
	 * not in the font are always stripped when typed or pasted.
	 */
	...
	/**
	 * Returns the text field's style. Modifying the returned style may not have
	 * an effect until {@link #setStyle(TextFieldStyle)} is called.
	 */
	...
	protected void calculateOffsets() {
		...
		if (style.background != null)
			visibleWidth -= style.background.getLeftWidth()
					+ style.background.getRightWidth();

		// Check if the cursor has gone out the left or right side of the
		// visible area and adjust renderoffset.
		...
		if (hasSelection) {
			...
			float maxX = Math.min(glyphPositions[maxIndex],
					glyphPositions[visibleTextEnd]);
			selectionX = minX;
			selectionWidth = maxX - minX;
		}

		if (rightAligned) {
			textOffset = visibleWidth
					- (glyphPositions[visibleTextEnd] - startPos);
			if (hasSelection)
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		final Color fontColor = (disabled && style.disabledFontColor != null) ? style.disabledFontColor
				: ((focused && style.focusedFontColor != null) ? style.focusedFontColor
						: style.fontColor);
		final Drawable selection = style.selection;
		final Drawable cursorPatch = style.cursor;
		final Drawable background = (disabled && style.disabledBackground != null) ? style.disabledBackground
				: ((focused && style.focusedBackground != null) ? style.focusedBackground
						: style.background);
		...
		if (displayText.length() == 0) {
			if (!focused && messageText != null) {
				...
				BitmapFont messageFont = style.messageFont != null ? style.messageFont
						: font;
				messageFont.draw(batch, messageText, x + bgLeftWidth, y + textY
						+ yOffset);
			}
		} else {
	...
	protected float getTextY(BitmapFont font, Drawable background) {
		...
		if (background != null) {
			...
			textY = (int) (textY
					+ (height - background.getTopHeight() - bottom) / 2 + bottom);
		} else {
	...
	protected void drawSelection(Drawable selection, Batch batch,
		...
		selection.draw(batch, x + selectionX + renderOffset, y - textHeight
				+ font.getDescent() / 2.0f, selectionWidth,
				textHeight + font.getDescent() / 2);
	}

	protected void drawText(Batch batch, BitmapFont font, float x, float y) {
		font.draw(batch, displayText, x + textOffset, y, visibleTextStart,
				visibleTextEnd);
	}

	protected void drawCursor(Drawable cursorPatch, Batch batch,
			BitmapFont font, float x, float y) {
		cursorPatch.draw(batch, x + textOffset + glyphPositions.get(cursor)
				- glyphPositions.items[visibleTextStart] - 1, y - textHeight
				+ font.getDescent() / 2.0f, cursorPatch.getMinWidth(),
				textHeight + font.getDescent() / 2);
	}
	...
	void updateDisplayText() {
		...
		font.computeGlyphAdvancesAndPositions(displayText, glyphAdvances,
				glyphPositions);
	...
	/**
	 * Copies the contents of this TextField to the {@link Clipboard}
	 * implementation set on this TextField.
	 */
	public void copy() {
		if (hasSelection && !passwordMode) {
			clipboard.setContents(text.substring(
	...
	/**
	 * Pastes the content of the {@link Clipboard} implementation set on this
	 * Textfield to this TextField.
	 */
	void paste() {
	...
	void paste(String content, boolean onlyFontChars) {
		...
		for (int i = 0, n = content.length(); i < n; i++) {
			...
			if (!(writeEnters && (c == ENTER_ANDROID || c == ENTER_DESKTOP))) {
				if (onlyFontChars && !style.font.containsCharacter(c))
					continue;
	...
	String insert(int position, CharSequence text, String to) {
		...
		return to.substring(0, position) + text
				+ to.substring(position, to.length());
	}
	...
	public void next(boolean up) {
		...
		TextField textField = findNextTextField(stage.getActors(), null, tmp2,
				tmp1, up);
		if (textField == null) { // Try to wrap around.
			...
			textField = findNextTextField(getStage().getActors(), null, tmp2,
					tmp1, up);
		}
		if (textField != null)
	...
	private TextField findNextTextField(Array<Actor> actors, TextField best,
			Vector2 bestCoords, Vector2 currentCoords, boolean up) {
			...
			if (actor instanceof TextField) {
				...
				Vector2 actorCoords = actor.getParent()
						.localToStageCoordinates(
								tmp3.set(actor.getX(), actor.getY()));
				if ((actorCoords.y < currentCoords.y || (actorCoords.y == currentCoords.y && actorCoords.x > currentCoords.x))
						^ up) {
					if (best == null
							|| (actorCoords.y > bestCoords.y || (actorCoords.y == bestCoords.y && actorCoords.x < bestCoords.x))
							^ up) {
	...
	public String getSelection() {
		return hasSelection ? text.substring(Math.min(selectionStart, cursor),
				Math.max(selectionStart, cursor)) : "";
	}
	...
	/**
	 * Sets the password character for the text field. The character must be
	 * present in the {@link BitmapFont}. Default is 149 (bullet).
	 */
	 ...
	 * instances. Just uses
	 * {@link com.badlogic.gdx.Input#setOnscreenKeyboardVisible(boolean)} as
	...
	public class TextFieldClickListener extends ClickListener {
		...
		public boolean keyDown(InputEvent event, int keycode) {
			...
			if (isMac)
				...
				ctrl = Gdx.input.isKeyPressed(Keys.CONTROL_LEFT)
				...
				if (add || remove) {
					...
					if (add && !remove) {
						...
						if (character != ENTER_DESKTOP
							...
							if (filter != null
									&& !filter.acceptChar(TextField.this,
											character))
								return true;
						}
	...
	static public class TextFieldStyle {
		...
		/** Optional. */
		public Drawable background, focusedBackground, disabledBackground,
				cursor, selection;
		/** Optional. */
		...
		public TextFieldStyle(BitmapFont font, Color fontColor,
				Drawable cursor, Drawable selection, Drawable background) {
