...
public class EditorIO implements LoadedCallback {
	...
	public EditorIO(Controller controller) {
		...
		this.editorAssets = controller.getEditorAssets();
	}
	 ...
	 * {@link es.eucm.ead.editor.assets.EditorAssets#GAME_FILE}, all
	 * {@link es.eucm.ead.editor.assets.EditorAssets#SCENES_PATH} have to be
	 * already loaded.
	 * 
	 ...
	 * {@link es.eucm.ead.editor.assets.EditorAssets#loadGame(com.badlogic.gdx.assets.AssetLoaderParameters.LoadedCallback)}
	 ...
	 *            {@link es.eucm.ead.editor.assets.EditorAssets} to resolve
	...
	private void saveGame(EditorGame game) {
		editorAssets.toJsonPath(game, EditorAssets.GAME_FILE);
	}

	private void saveScenes(Map<String, EditorScene> scenes) {
		for (Map.Entry<String, EditorScene> entry : scenes.entrySet()) {
			editorAssets.toJsonPath(entry.getValue(),
					editorAssets.convertSceneNameToPath(entry.getKey()));
		}
	}

	/**
	 * Removes all json files from disk under the
	 * {@link es.eucm.ead.editor.assets.EditorAssets#getLoadingPath()} folder.
	 * 
	 * 
	...
	private void removeAllJsonFilesPersistently() {
		String loadingPath = controller.getEditorAssets().getLoadingPath();
		deleteJsonFilesRecursively(controller.getEditorAssets().absolute(
				loadingPath));
	}
	...
	public void finishedLoading(AssetManager assetManager, String fileName,
		...
		if (type == EditorScene.class) {
			String sceneId = editorAssets.resolve(fileName)
