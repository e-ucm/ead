...
/**
 * 
 * Transforms the selected entities into a group
 * 
 */
public class GroupSelection extends ModelAction implements SelectionListener {
	...
	public Command perform(Object... args) {
		...
		command.addCommand(actions.getAction(AddSceneElement.class).perform(
				group));

		for (Object object : selection.get(Selection.SCENE_ELEMENT)) {
			ModelEntity element = (ModelEntity) object;
			command.addCommand(actions.getAction(RemoveChildFromEntity.class)
					.perform(context, element));
			command.addCommand(actions.getAction(AddChildToEntity.class)
					.perform(element, group));
		}
