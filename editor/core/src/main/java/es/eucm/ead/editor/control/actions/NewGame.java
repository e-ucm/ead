...
import es.eucm.ead.editor.view.options.FileOption;
...
public class NewGame extends EditorAction implements DialogListener {
	...
	public NewGame() {
		...
		defaultGame.setTitle(I18N.m("game.title.default"));
	...
	public void perform() {
		...
		Editor.controller.getViewController().showOptionsDialog(op, this,
	...
	private void createGame() {
		...
		gamePath.mkdirs();
		if (!gamePath.exists()) {
			Editor.controller.getViewController().showError("game.error.path",
					this);
			return;
		}
		...
		Engine.engine.setLoadingPath(scenesFolder.file().getAbsolutePath());

		Editor.sceneManager.loadGame();
		Editor.platform.setTitle(I18N.m("editor.title.game", defaultGame
				.getTitle()));

		DependencyNode node = new DependencyNode(EditorModel.gameId,
				defaultGame);
		Editor.controller.getModel().registerNode(node);
	}
